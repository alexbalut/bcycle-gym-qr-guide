...
Members never log in. `/` is the in-product gym home for **b.cycle Montreal**.
...
3. Friendly URLs: `/m/bcycle/lat-pulldown`.
...
Keyed by gym slug (`bcycle`). No member account.

| Key | Contents |
|-----|----------|
| `bcycle-workout:v1:<slug>` | In-progress session JSON |
| `bcycle-workout-history:v1:<slug>` | Array of saved workouts (newest first) |
