...
Seed gym: `bcycle`, `primaryColor: #E91E63`, `secondaryColor: #000000`.

## Multi-tenant gym

Admin session carries `gymId`. APIs scope mutations to that gym. Member home loads slug `bcycle` (fallback: first gym).
...
| Workout + history | localStorage keys `bcycle-workout:v1:<slug>` and `bcycle-workout-history:v1:<slug>` |
